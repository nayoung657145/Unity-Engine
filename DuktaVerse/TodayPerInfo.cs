...
public class TodayPerInfo : MonoBehaviour
{
    public string[] info;
    public TextMeshProUGUI infoText;

    public static bool concertType2;    //어떤 공연장인지 구분하기 위해서
    void Start()
    {
    ...
    public void ClickInfoIcon ()
    {
        ...
        {
            infoText.text = "Song 공연장에 예정된 공연이 없습니다.";
        }
            
    }
    private IEnumerator GetMySQLData()
    {
        string serverPath = "http://3.35.93.147/TodayPerInfo.php"; //PHP 파일의 위치를 저장
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Post(serverPath, form)) //웹 서버에 요청
        {
            ...
            infoText.text = "\'" + webRequest.downloadHandler.text + "\'" + "이(가) Song 공연장에서 있을 예정입니다.";
